...
migrate-up:
	migrate -source file://$(subst \,/,$(MPATH)) -database postgres://postgres:0000@localhost:$(PORT)/postgres?sslmode=disable up $(N)

migrate-down:
	migrate -source file://$(subst \,/,$(MPATH)) -database postgres://postgres:0000@localhost:$(PORT)/postgres?sslmode=disable down $(N)

migrate-force:
	migrate -source file://$(subst \,/,$(MPATH)) -database postgres://postgres:0000@localhost:$(PORT)/postgres?sslmode=disable force $(VERSION)